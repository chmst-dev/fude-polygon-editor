...
-- 初期作業項目の投入（冪等: ON CONFLICT DO NOTHING）
...
CREATE INDEX IF NOT EXISTS field_work_records_field_id_idx
  ON public.field_work_records (field_id);

CREATE INDEX IF NOT EXISTS field_work_records_latest_idx
  ON public.field_work_records (field_id, worked_on DESC NULLS LAST, created_at DESC, id DESC);

CREATE INDEX IF NOT EXISTS field_work_records_work_type_idx
  ON public.field_work_records (work_type_id);
...
-- --------------------------------------------------------
-- merge_fields: 登録済み圃場統合 (単一トランザクション)
-- --------------------------------------------------------
...
BEGIN
  ...
  -- 6. field_source_polygons を target へ移動
...
-- --------------------------------------------------------
-- get_latest_work_records: 圃場ごとの最新作業を一括取得 (組織・共有トークン制限付き)
-- --------------------------------------------------------
...
-- --------------------------------------------------------
-- get_field_work_records: 特定圃場の全作業履歴を取得 (組織・共有トークン制限付き)
-- --------------------------------------------------------
...
DROP POLICY IF EXISTS "work_types_delete_admin" ON public.work_types;
CREATE POLICY "work_types_delete_admin" ON public.work_types FOR DELETE USING (public.get_my_role() = 'admin');

-- field_work_records: 自組織の圃場に属するレコードのみ参照可
...
CREATE POLICY "fwr_select_own_org" ON public.field_work_records FOR SELECT USING (
  ...
  AND (public.get_my_role() = 'admin' OR EXISTS (
    SELECT 1 FROM public.fields f
...
-- INSERT/UPDATE/DELETE: admin/org_admin のみ許可 (viewer 制限)
DROP POLICY IF EXISTS "fwr_insert_own_org" ON public.field_work_records;
CREATE POLICY "fwr_insert_own_org" ON public.field_work_records FOR INSERT WITH CHECK (
  auth.role() = 'authenticated'
  AND public.get_my_role() IN ('admin', 'org_admin')
  AND auth.uid() = created_by
  AND (public.get_my_role() = 'admin' OR EXISTS (
    SELECT 1 FROM public.fields f
    WHERE f.id = field_work_records.field_id AND f.organization_id = public.get_my_org_id()
  ))
);

DROP POLICY IF EXISTS "fwr_update_own" ON public.field_work_records;
...
DROP POLICY IF EXISTS "fwr_delete_own" ON public.field_work_records;
...
-- ============================================================
-- PART 4: 既存テーブルのRLS制限強化 (viewer = 閲覧専用)
-- ============================================================
...
-- source_polygons
DROP POLICY IF EXISTS "source_polygons_insert_authenticated" ON public.source_polygons;
...
DROP POLICY IF EXISTS "source_polygons_update_authenticated" ON public.source_polygons;
...
-- ============================================================
-- PART 5: 共有トークン認証用の新規RPCとRLSの追加・更新
-- ============================================================

-- share_links の RLS 有効化とポリシー
...
-- anonの直接 SELECT 封鎖と認証済みユーザー制限の更新
-- 1. organizations
